...
export async function up(db: Kysely<unknown>): Promise<void> {
  await addIdColumn(db.schema.createTable('returns'), db)
    ...
    .addColumn('refund_total', 'double precision', (c) => c.notNull().defaultTo(0))
  ...
  await addIdColumn(db.schema.createTable('return_items'), db)
    ...
    .addColumn('refund_amount', 'double precision', (c) => c.notNull())
    .execute()
  await db.schema
