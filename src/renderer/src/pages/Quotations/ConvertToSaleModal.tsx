...
import type { PaymentInput, PaymentMethod, ReceiptData } from '@shared/sales'
...
import styles from '../Sales/PaymentModal.module.css'
...
export function ConvertToSaleModal({
  ...
  onClose,
  onComplete
}: ConvertToSaleModalProps): JSX.Element {
  ...
  useEffect(() => {
    if (quotation.customer_id == null) return
    window.api.customers.get(quotation.customer_id).then((customer) => {
      if (!customer) return
      setCanUseCredit(Boolean(customer.is_credit_account))
      setAvailableCredit(customer.credit_limit - customer.credit_balance)
    })
  ...
  async function handleConfirm(): Promise<void> {
    ...
    try {
      ...
      const data = await window.api.quotations.convertToSale(quotation.id, { payments }, uiLanguage)
      onComplete(data)
    } catch (err) {
  ...
  return (
    <Modal
      title={t('quotations.actions.convertToSale')}
      onClose={onClose}
      footer={
        <>
